...
> 中立、免费的 AI 套餐对比站。覆盖 Coding / Token / Video / Image / Audio 五大场景 + 已下架归档，55 家展示平台（另有 1 家中转站数据保留未上线）、95 个套餐档位，一个页面全搞定。
...
- **12 列对比大表**：平台 / 方案 / 开通 / 月付 / 季付折月均 / 年付折月均 / 5h请求数 / 周请求数 / 月总请求数 / 支持模型 / 额外权益 / 备注
- **多维度筛选**：平台下拉、模型下拉、月付/季付折月均/年付折月均上限、计费方式（全部/Coding/Token/Credits），组合生效
- **跨币种折算**：美元套餐自动按 7.2 汇率折算人民币参与排序和价格筛选，显示 `$20 ≈¥144`
- **排行徽章**：入门 / 性价比 / 进阶 / 旗舰 / 活动 / 免费 / 按量 / 售罄 / 团队，同分内可置顶 featured 平台
...
| 分类 | 平台数 | 说明 |
|---|---|---|
| 💻 Coding | 27 | 按次订阅的编程套餐（智谱/稳明/字节方舟/Kimi/百度/腾讯…） |
...
| 📦 已下架 | 1 | 停服平台归档（京东云） |
| 🚇 中转站 | 1 | 数据保留但未上线（幻境 MirageAI，不展示） |

表内展示平台合计 55 家（Coding 27 + Token 9 + Video 8 + Image 6 + Audio 4 + 已下架 1）；另有 1 家中转站（relay-mirage）数据保留未上线，数据总量 56 家。

Video / Image / Audio / 已下架 / 中转站 类别暂仅含平台概览或保留数据，无套餐明细，数据持续整理中。
...
├── docs/
│   └── AIPlanHub-幻境MirageAI-技术分析文档.md   # 外部参考资料/调研存档，非本项目代码文档
├── .gitignore
└── README.md
```
...
- **6 个平台使用站方邀请码**：稳明光语纪、智谱 AI、字节·方舟、Kimi、OpenStarry、xKiro
...
- 本站**不售卖任何套餐**，仅提供信息对比，以各厂商官方页面为准；部分平台使用站方邀请码/返利链接（策略透明，见上「🔗 开通链接策略」），其余一律官方直链
- 数据仅供参考，不构成购买建议
